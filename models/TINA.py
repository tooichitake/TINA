...
class SelfEncoderLayer(nn.Module):
    def __init__(self, self_attention, d_model, d_ff=None, dropout=0.1, activation="gelu"):
...
class EnEmbedding(nn.Module):
    """Embedding for TINA"""
    def __init__(self, d_model, patch_len, dropout, n_vars, use_time_features=False, freq='h'):
    ...
    def forward(self, x, x_mark=None):
        ...
        var_embed = self.variable_embedding(var_ids)  # [n_vars, d_model]
        var_embed = var_embed.unsqueeze(0).expand(B, -1, -1)  # [B, n_vars, d_model]
        var_embed = var_embed.reshape(B * n_vars, 1, -1)  # [B*n_vars, 1, d_model]
        x_embed = x_embed + var_embed  # broadcast to [B*n_vars, patch_num, d_model]
